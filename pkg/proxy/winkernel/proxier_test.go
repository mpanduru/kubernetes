...
// TestCreateServiceVip verifies if the ServiceVip is succesfully created as expected
...
// succesfully created on a network with type OVERLAY
...
// TestCreateRemoteEndpointOverlay verifies if the remote endpoint is
// succesfully created on a network with type L2BRIDGE
...
func TestSharedRemoteEndpointDelete(t *testing.T) {
	...
	// Assert --- here we verify that the endpoint was succesfully deleted
	ep = proxier.endpointsMap[svcPortName1][0]
	epInfo, ok = ep.(*endpointsInfo)
	if !ok {
		t.Errorf("Failed to cast endpointsInfo %q", svcPortName1.String())
...
// TestEndpointSlice checks if endpoint slices are initialized succesfully
...
// and checks if it is succesfully updated. Also the test checks that the service map
